...
#if !defined(FIXEDBGTENSORFIELDOLD_HPP_)
#error "This file should only be included through FixedBGTensorField.hpp"
#endif

#ifndef FIXEDBGTENSORFIELDOLD_IMPL_HPP_
#define FIXEDBGTENSORFIELDOLD_IMPL_HPP_
    ...
    const diff2_vars_t<Tensor<2, data_t>> &d2, const vars_t<data_t> &advec)
{
   ...
   // rhs.Pi = metric_vars.lapse * metric_vars.K * vars.Pi + advec.Pi;
    ...
    FOR2(i,j)
    {
        tensorRiemannTerm[i][j] = //vars.fhat * metric_vars.K_tensor[i][j] * metric_vars.K
                                    ...
                                    (-vars.fhat * metric_vars.K_tensor[i][j] * metric_vars.K 
                                    - vars.fhat * metric_vars.ricci_phys[i][j]) / metric_vars.lapse; 
        FOR1(k)
        {
            ...
            FOR1(l)
            {
                ...
                + vars.fhat * gamma_UU[k][l] * metric_vars.K_tensor[i][l] * metric_vars.K_tensor[j][k] / metric_vars.lapse;
                FOR2(m,n)
                {
    ...
    */
    ...
    rhs.fhat = advec.fhat + metric_vars.lapse * metric_vars.lapse * vars.w;
    ...
    rhs.w = advec.w + metric_vars.lapse * metric_vars.K * vars.w - temp_mass * temp_mass * vars.fhat - 2.0 * metric_vars.lapse * scalarRiemannTerm;

    //1 summation index
    FOR1(i)
    {   
        //rhs.fbar[i] = advec.fbar[i] - vars.fhat * metric_vars.d1_lapse[i] - metric_vars.lapse * vars.q[i];
        rhs.fbar[i] = advec.fbar[i] + (1.0 / metric_vars.lapse) * vars.fhat * metric_vars.d1_lapse[i] - metric_vars.lapse * vars.q[i];
    ...
    */
    ...
    data_t damping_param_1 = 10.0;
    data_t damping_param_2 = 1.0;

    //Fifth damping implementation
    /*
    rhs.fhat += -0.5 * metric_vars.lapse * traceFieldConstraint;
    rhs.w += -0.5 * metric_vars.lapse * traceMomentumConstraint;
    */
    /*
    ...
    */
    ...
    rhs.thetahat = 0.1 * metric_vars.lapse * (primaryScalar - damping_param_1 * (2.0 + damping_param_2) * vars.thetahat);
    
    FOR1(i)
    {
        rhs.thetaspatial[i] = 0.1 * metric_vars.lapse * (primaryVector[i] - damping_param_1 * vars.thetaspatial[i]);// - vars.thetahat * metric_vars.d1_lapse[i];
...
*/
#endif /* FIXEDBGTENSORFIELDOLD_IMPL_HPP_ */
