...
class TaubFixedBG
{
  public:
                                   ...
                                   const Cell<data_t> &current_cell) const
    {
        ...
        vars.lapse = pow(z, -1/3);
        FOR2(i, j)
        {
            vars.gamma[i][j] = 0.0;
                
        }
        vars.gamma[0][0] = pow(z,4/3);
        vars.gamma[1][1] = pow(z,4/3);
        vars.gamma[2][2] = 1.0;
        ...
        FOR1(k)
        {
            vars.d1_gamma[0][0][k] = (4/3) * delta(k,2) * pow(z,1/3);
            vars.d1_gamma[1][1][k] = (4/3) * delta(k,2) * pow(z,1/3);
        }
        ...
        FOR1(k)
        {
            vars.d1_gamma_UU[0][0][k] = (-4/3) * delta(k,2) * pow(z,-7/3); 
            vars.d1_gamma_UU[1][1][k] = (-4/3) * delta(k,2) * pow(z,-7/3); 
        }
        ...
        FOR2(k,m)
        {
            vars.d2_gamma[0][0][k][m] = (4/9) * delta(k,2) * delta(m,2) * pow(z,-2/3);
            vars.d2_gamma[1][1][k][m] = (4/9) * delta(k,2) * delta(m,2) * pow(z,-2/3);
        }
        ...
        FOR1(i)
        {
            vars.d1_lapse[i] = (-1/3) * delta(i,2) * pow(z,-4/3);
        }

        FOR2(i,j)
        {
            vars.d2_lapse[i][j] = (4/9) * delta(i,2) * delta(j,2) * pow(z,-7/3);
        }
