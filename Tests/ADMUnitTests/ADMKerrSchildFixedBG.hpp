...
#ifndef ADMKERRSCHILDFIXEDBG_HPP_
#define ADMKERRSCHILDFIXEDBG_HPP_
...
class ADMKerrSchildFixedBG
{
  public:
    ...
    ADMKerrSchildFixedBG(params_t a_params, double a_dx)
                                   ...
                                   const Coordinates<double> &coords) const
    {
        ...
        // Manually define its contents as there isn't any nice symmetry we can
        // use
        ...
        FOR3(i, j, k)
        {
            ...
            vars.d1_gamma_UU[i][j][k] =
                -2.0 * dHdx[k] *
                    (TensorAlgebra::delta(i, j) + 2.0 * H * lambda_UU[i][j]) /
                    ((1 + 2.0 * H) * (1 + 2.0 * H)) +
                ((2.0 * dHdx[k] * lambda_UU[i][j]) +
                 (2.0 * H * d1_lambda_UU[i][j][k])) /
                    (1 + 2.0 * H);

            // vars.d1_gamma_UU[i][j][k] = 0;
            /*
            vars.d1_gamma_UU[i][j][k] = 2.0 * (dHdx[k] * el[i] * el[j] + H *
            dldx[i][k] * el[j] + H * el[i] * dldx[j][k])
            - 2.0 * pow(vars.lapse,-3) * vars.d1_lapse[k] * vars.shift[i] *
            vars.shift[j]
            + (vars.d1_shift[i][k] * vars.shift[j] + vars.shift[i] *
            vars.d1_shift[j][k])/ (vars.lapse * vars.lapse);
            */
        }
        ...
        FOR1(i)
        {
            FOR3(j, k, m)
            {
                vars.d2_gamma[i][j][k][m] =
                    2.0 *
                    (d2Hdx2[k][m] * el[i] * el[j] +
                     dHdx[k] * dldx[i][m] * el[j] +
                     dHdx[k] * el[i] * dldx[j][m] +
                     dHdx[m] * dldx[i][k] * el[j] +
                     H * d2ldx2[i][k][m] * el[j] + H * dldx[i][k] * dldx[j][m] +
                     dHdx[m] * el[i] * dldx[j][k] +
                     H * dldx[i][m] * dldx[j][k] + H * el[i] * d2ldx2[j][k][m]);
        ...
        {
            vars.d2_lapse[i][j] =
                -3.0 * pow(vars.lapse, 2.0) * vars.d1_lapse[j] * el_t *
                    (el_t * dHdx[i] + 2.0 * H * dltdx[i])

                - pow(vars.lapse, 3.0) * (2.0 * el_t * dltdx[j] * dHdx[i] +
                                          el_t * el_t * d2Hdx2[i][j] +
                                          2.0 * el_t * dHdx[j] * dltdx[i] +
                                          2.0 * H * el_t * d2ltdx2[i][j]);
        }
  ...
  protected:
                       ...
                       const data_t &H, const Coordinates<data_t> &coords) const
    {
        ...
        FOR2(i, j)
        {
            d2rhodx2[i][j] = delta(i, j) / rho - x[i] * x[j] / pow(rho, 3.0);
        }
        ...
        const data_t denom =
            pow(0.25 * (rho2 - a2) * (rho2 - a2) + a2 * z * z, 0.5);
        Tensor<1, data_t> d1_denom;
        FOR1(i)
        {
            d1_denom[i] =
                0.5 *
                (drhodx[i] * rho * (rho2 - a2) + 2.0 * delta(i, 2) * a2 * z) /
                denom;
        }
        ...
        FOR2(i, j)
        {
            ...
            d2rdx2[i][j] =
                -1.0 / r * drdx[j] * drdx[i] +
                0.5 / r * (drhodx[i] * drhodx[j] + rho * d2rhodx2[i][j]) +
                (0.25 / r) *
                    (d2rhodx2[i][j] * rho * (rho2 - a2) +
                     drhodx[i] * drhodx[j] * (rho2 - a2) +
                     2.0 * drhodx[i] * drhodx[j] * rho2 +
                     2.0 * delta(i, 2) * delta(j, 2) * a2) *
                    pow(0.25 * (rho2 - a2) * (rho2 - a2) + a2 * z * z, -0.5) -
                (0.25 / r) *
                    (drhodx[i] * rho * (rho2 - a2) +
                     2.0 * delta(i, 2) * a2 * z) *
                    0.5 *
                    (drhodx[j] * rho * (rho2 - a2) +
                     2.0 * delta(j, 2) * a2 * z) *
                    pow(0.25 * (rho2 - a2) * (rho2 - a2) + a2 * z * z, -1.5);
        }
        ...
        FOR2(i, j)
        {
            d2costhetadx2[i][j] =
                (1.0 / r2) *
                (-delta(i, 2) * drdx[j] - delta(j, 2) * drdx[i] +
                 (2.0 / r) * z * drdx[i] * drdx[j] - z * d2rdx2[i][j]);
        }
        ...
        FOR2(i, j)
        {
            d2Hdx2[i][j] =
                ...
                H * (d2rdx2[i][j] / r - drdx[i] * drdx[j] / r2 +
                     4.0 * (r * drdx[j] + a2 * dcosthetadx[j] * cos_theta) *
                         (r * drdx[i] + a2 * dcosthetadx[i] * cos_theta) /
                         ((r2 + a2 * cos_theta2) * (r2 + a2 * cos_theta2)) -
                     2.0 *
                         (drdx[j] * drdx[i] + r * d2rdx2[i][j] +
                          a2 * dcosthetadx[i] * dcosthetadx[j] +
        ...
        FOR2(i, j)
        {
            // el_x component
            /*
            d2ldx2[0][i][j] = delta(j,0) * drdx[i] + x[0] * d2rdx2[i][j] +
            drdx[j] * delta(i,0) +
             ((-2.0 * drdx[i] * drdx[j] - 2.0 * r * d2rdx2[i][j]) * (r * x[0] +
            a * x[1]) - 2.0 * r * drdx[i] * (drdx[j] * x[0] + r * delta(j,0) + a
            * delta(j,1) )) / (r2 + a2) / (r2 + a2)
            + 8.0 * r2 * drdx[i] * drdx[j] * (r * x[0] + a * x[1]) * pow((r2 +
            a2),-3.0);
            */
            d2ldx2[0][i][j] =
                ...
                2.0 * drdx[j] * r *
                    (x[0] * drdx[i] + r * delta(i, 0) + a * delta(i, 1)) /
                    ((r2 + a2) * (r2 + a2)) +
                (-2.0 * drdx[j] * drdx[i] * (r * x[0] + a * x[1]) -
                 2.0 * r * d2rdx2[i][j] * (r * x[0] + a * x[1]) -
                 2.0 * r * drdx[i] *
                     (drdx[j] * x[0] + r * delta(j, 0) + a * delta(j, 1))) /
                    ((r2 + a2) * (r2 + a2)) +
                8.0 * r2 * drdx[i] * drdx[j] * (r * x[0] + a * x[1]) *
                    pow((r2 + a2), -3.0);
            // el_y component
            /*
            d2ldx2[1][i][j] = delta(j,1) * drdx[i] + x[1] * d2rdx2[i][j] +
            drdx[j] * delta(i,0) +
            ((-2.0 * drdx[i] * drdx[j] - 2.0 * r * d2rdx2[i][j]) * (r * x[1] - a
            * x[0]) - 2.0 * r * drdx[i] * (drdx[j] * x[1] + r * delta(j,1) - a *
            delta(j,0) )) / (r2 + a2) / (r2 + a2)
            + 8.0 * r2 * drdx[i] * drdx[j] * (r * x[1] - a * x[0]) * pow((r2 +
            a2),-3.0);
            */
            d2ldx2[1][i][j] =
                ...
                2.0 * drdx[j] * r *
                    (x[1] * drdx[i] + r * delta(i, 1) - a * delta(i, 0)) /
                    ((r2 + a2) * (r2 + a2)) +
                (-2.0 * drdx[j] * drdx[i] * (r * x[1] - a * x[0]) -
                 2.0 * r * d2rdx2[i][j] * (r * x[1] - a * x[0]) -
                 2.0 * r * drdx[i] *
                     (drdx[j] * x[1] + r * delta(j, 1) - a * delta(j, 0))) /
                    ((r2 + a2) * (r2 + a2)) +
                8.0 * r2 * drdx[i] * drdx[j] * (r * x[1] - a * x[0]) *
                    pow((r2 + a2), -3.0);
            // el_z component
            d2ldx2[2][i][j] =
                -(delta(j, 2) * drdx[i] + x[2] * d2rdx2[i][j] -
                  2.0 * x[2] * drdx[i] * drdx[j] / r + delta(i, 2) * drdx[j]) /
                r2;
        }
...
#endif /* ADMKERRSCHILDFIXEDBG_HPP_ */
