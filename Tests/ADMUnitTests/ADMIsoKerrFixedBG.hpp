...
class ADMIsoKerrFixedBG
{
  ...
  public:
    ...
    */
    ...
    void compute_metric_background(vars_t<data_t> &vars, const Coordinates<double> &coords) const
    {
        // where am i?
        //const Coordinates<data_t> coords(current_cell, m_dx, m_params.center);
        ...
        FOR3(i,j,k)
        {
            d2jacdx2[0][i][j][k] = -delta(i,j) * dRdx[k] / R2 - delta(i,k) * x_i[j] / R2 /R - delta(j,k) * x_i[i] /R2 /R + 3.0 *  x_i[i] * x_i[j] * dRdx[k] / (R2 * R2);
        }

        FOR2(i,j)
        {
            d2jacdx2[1][0][i][j] = (-2.0 * dRdx[j] / R - drhodx[j] / rho) /R2 / rho * (delta(i,0) * z + delta(i,2) * x - x * z * drhodx[i]/rho - 2.0 * x * z * dRdx[i]/R)
                                    + 1.0 / R2 / rho * (delta(i,0) * delta(j,2) + delta(i,2) * delta(j,0) - (delta(j,0) * z + x * delta(j,2))* drhodx[i]/rho
                                                                                                        -x * z * d2rhodx2[i][j] / rho + x * z * drhodx[i] * drhodx[j]/ rho2
                                                                                                    - 2.0 * (delta(j,0) * z + x * delta(j,2)) * dRdx[i]/R 
                                                                                                    -2.0 * x * z * d2Rdx2[i][j] / R + 2.0 * x * z * dRdx[i] * dRdx[j] /R2);

            /*1.0 / R2 / rho * (delta(i,0) * delta(j,2) + delta(i,2) * delta(j,0)) 
                                    - (2.0 * dRdx[j]/R + drhodx[j]/rho) * (delta(i,0) * z + delta(i,2) * x 
                                    -x * z * drhodx[i] / rho - 2.0 * dRdx[i] * x * z / R) / R2 / rho
            ...
            d2jacdx2[1][1][i][j] = (-2.0 * dRdx[j] / R - drhodx[j] / rho) /R2 / rho * (delta(i,1) * z + delta(i,2) * y - y * z * drhodx[i]/rho - 2.0 * y * z * dRdx[i]/R)
                                    + 1.0 / R2 / rho * (delta(i,1) * delta(j,2) + delta(i,2) * delta(j,1) - (delta(j,1) * z + y * delta(j,2))* drhodx[i]/rho
                                    -y * z * d2rhodx2[i][j] / rho + y * z * drhodx[i] * drhodx[j]/ rho2
                                                                                                    - 2.0 * (delta(j,1) * z + y * delta(j,2)) * dRdx[i]/R 
                                    -2.0 * y * z * d2Rdx2[i][j] / R + 2.0 * y * z * dRdx[i] * dRdx[j] /R2);
            ...
            //djacdx[2][1][i] = (+delta(i, 0) - 2.0 * x * drhodx[i] / rho) / rho2;
            //djacdx[2][2][i] = 0.0;
            d2jacdx2[2][0][i][j] = 2.0 * (delta(i,1) * drhodx[j] + delta(j,1) * drhodx[i] + y * d2rhodx2[i][j] - 3.0 * y * drhodx[i] * drhodx[j] / rho) /rho2 / rho; 

            d2jacdx2[2][1][i][j] = -2.0 * (delta(i,0) * drhodx[j] + delta(j,0) * drhodx[i] + x * d2rhodx2[i][j] - 3.0 * x * drhodx[i] * drhodx[j] / rho) /rho2 / rho; 
        ...
        FOR1(i)
        {
            dDeltadR_ddx[i] = drBLdR_ddx[i] * (2.0 * r_BL - 2.0 * M) + drBLdR * (2.0 * drBLdx[i]);
        } 
        ...
        FOR1(i)
        {
            dAAdR_ddx[i] = 4.0 * (drBLdR_ddx[i] * r_BL2 * r_BL + 3.0 * drBLdR * r_BL2 * drBLdx[i] + drBLdR_ddx[i] * r_BL * a2 + drBLdR * drBLdx[i] * a2)
                                            -dDeltadR_ddx[i] * a2 * sin_theta2 - dDeltadR * a2 * 2.0 * sin_theta * dsin_thetadx[i]; 
        } 
        Tensor<1, data_t> dgammaRRdR_ddx;
        FOR1(i)
        {
            dgammaRRdR_ddx[i] = dgammaRRdx[i] * dgammaRRdR / gamma_RR
            + gamma_RR * (dSigmadR_ddx[i] / Sigma - dSigmadx[i] * dSigmadR / Sigma2 + dRdx[i] / R2 - drBLdR_ddx[i] / (r_BL - r_minus)
            + drBLdR * drBLdx[i] / (r_BL - r_minus) / (r_BL - r_minus) + r_plus * dRdx[i] / R2 / R / (1.0 + 0.25 * r_plus / R) - 0.125 * r_plus * r_plus / R2 / R2 * dRdx[i] / (1.0 + 0.25 * r_plus/ R) / (1.0 + 0.25 * r_plus/ R));
        }
        ...
        */
        ...
        FOR2(i,j)
        {
            ...
            d2Deltadx2[i][j] = djacdx[0][i][j] * dDeltadR + jac[0][i] * dDeltadR_ddx[j];
            d2AAdx2[i][j] = djacdx[0][i][j] * dAAdR + jac[0][i] * dAAdR_ddx[j] + djacdx[1][i][j] * dAAdtheta + jac[1][i] * dAAdtheta_ddx[j];         
            d2gammaRRdx2[i][j] = djacdx[0][i][j] * dgammaRRdR + jac[0][i] * dgammaRRdR_ddx[j] + djacdx[1][i][j] * dgammaRRdtheta + jac[1][i] * dgammaRRdtheta_ddx[j];  
            d2gammappdx2[i][j] = djacdx[0][i][j] * dgammappdR + jac[0][i] * dgammappdR_ddx[j] + djacdx[1][i][j] * dgammappdtheta + jac[1][i] * dgammappdtheta_ddx[j]; 
        ...
        FOR3(i,j,k)
        {
            FOR1(l)
            {
                vars.d2_gamma[i][j][k][l] =   d2jacdx2[0][j][k][l] * jac[0][i] * gamma_RR + djacdx[0][j][k] * djacdx[0][i][l] * gamma_RR + djacdx[0][j][k] * jac[0][i] * dgammaRRdx[l]
                                            ...
                                            + d2jacdx2[2][j][k][l] * jac[2][i] * gamma_pp + djacdx[2][j][k] * djacdx[2][i][l] * gamma_pp + djacdx[2][j][k] * jac[2][i] * dgammappdx[l]
                                            + d2jacdx2[2][i][k][l] * jac[2][j] * gamma_pp + djacdx[2][i][k] * djacdx[2][j][l] * gamma_pp + djacdx[2][i][k] * jac[2][j] * dgammappdx[l]
        ...
        FOR1(i)
        {
            vars.d1_gamma_UU[0][0][i] = -2.0 * drhodx[i] / rho * gamma_UU[0][0] + 1.0 / rho2 * (2.0 * (y * delta(i,1) * rho2 + rho * drhodx[i] * y2)/gamma_pp - y2 * rho2 * dgammappdx[i] / gamma_pp / gamma_pp + 2.0 * x * delta(i,0) / R2 / R2 / Sigma / gamma_RR * (R2 * rho2 * Sigma + R2 * R2 * z2 * gamma_RR)
                                        + x2 / R2 /R2 / Sigma / gamma_RR * (2.0 * R * dRdx[i] * rho2 * Sigma + 2.0 * R2 * rho * drhodx[i] * Sigma + R2 * rho2 * dSigmadx[i] + 4.0 * R2 * R * dRdx[i] * z2 * gamma_RR 
                                        + 2.0 * R2 * R2 * z * delta(i,2) * gamma_RR + R2 * R2 * z2 * dgammaRRdx[i]) + x2 *(R2 * rho2 * Sigma + R2 * R2 * z2 * gamma_RR) * (-4.0 * dRdx[i] / R2 / R2 / R / Sigma / gamma_RR - dSigmadx[i] / R2 /R2 / Sigma2 / gamma_RR - dgammaRRdx[i] / R2 / R2 / Sigma / gamma_RR / gamma_RR));
                        
            vars.d1_gamma_UU[0][1][i] = (delta(i,0) / x  + delta(i,1) / y - 2.0 * drhodx[i] / rho) * gamma_UU[0][1] + x * y / rho2 * (-2.0 * drhodx[i] * rho / gamma_pp + rho2 * dgammappdx[i] / gamma_pp / gamma_pp + (2.0 * dRdx[i] * R * rho2 * Sigma + 2.0 * R2 * drhodx[i] * rho * Sigma + R2 * rho2 * dSigmadx[i]
                                        + 4.0 * R2 * R * dRdx[i] * z2 * gamma_RR + 2.0 * R2 * R2 * z * delta(i,2) * gamma_RR + R2 * R2 * z2 * dgammaRRdx[i]) /R2 / R2 / Sigma / gamma_RR + (-4.0 * dRdx[i] / R2 /R2 / R / Sigma / gamma_RR - dSigmadx[i] / R2 / R2 / Sigma2 / gamma_RR - dgammaRRdx[i] / R2 / R2 / Sigma / gamma_RR / gamma_RR) * (R2 * rho2 * Sigma + R2 * R2 * z2 * gamma_RR));

            vars.d1_gamma_UU[0][2][i] = (delta(i,0) / x + delta(i,2) / z - 2.0 * dRdx[i] / R - dSigmadx[i] / Sigma - dgammaRRdx[i] / gamma_RR) * gamma_UU[0][2] 
                                        - x * z / R2 / Sigma / gamma_RR * (-dSigmadx[i] + 2.0 * R * dRdx[i] * gamma_RR + R2 * dgammaRRdx[i]);
        
            vars.d1_gamma_UU[1][1][i] = -2.0 * drhodx[i] / rho * gamma_UU[1][1] + 1.0 / rho2 * (2.0 * (x * delta(i,0) * rho2 + rho * drhodx[i] * x2)/gamma_pp - x2 * rho2 * dgammappdx[i] / gamma_pp / gamma_pp + 2.0 * y * delta(i,1) / R2 / R2 / Sigma / gamma_RR * (R2 * rho2 * Sigma + R2 * R2 * z2 * gamma_RR)
                                        + y2 / R2 /R2 / Sigma / gamma_RR * (2.0 * R * dRdx[i] * rho2 * Sigma + 2.0 * R2 * rho * drhodx[i] * Sigma + R2 * rho2 * dSigmadx[i] + 4.0 * R2 * R * dRdx[i] * z2 * gamma_RR 
                                        + 2.0 * R2 * R2 * z * delta(i,2) * gamma_RR + R2 * R2 * z2 * dgammaRRdx[i]) + y2 *(R2 * rho2 * Sigma + R2 * R2 * z2 * gamma_RR) * (-4.0 * dRdx[i] / R2 / R2 / R / Sigma / gamma_RR - dSigmadx[i] / R2 /R2 / Sigma2 / gamma_RR - dgammaRRdx[i] / R2 / R2 / Sigma / gamma_RR / gamma_RR));

            vars.d1_gamma_UU[1][2][i] = gamma_UU[1][2] * (-2.0 * dRdx[i] / R - dSigmadx[i]/Sigma - dgammaRRdx[i] / gamma_RR + delta(i,1) / y + delta(i,2) / z - 2.0 * drhodx[i] / rho + 2.0 * drhodx[i] / rho)
                                        - y * z / R2 / Sigma / gamma_RR * (-dSigmadx[i] + 2.0 * dRdx[i] * R * gamma_RR + R2 * dgammaRRdx[i]); 
            
            vars.d1_gamma_UU[2][2][i] = gamma_UU[2][2] * (-2.0 * dRdx[i] / R - dSigmadx[i]/Sigma - dgammaRRdx[i] / gamma_RR)
                                        + 1.0 / R2 / Sigma / gamma_RR * (2.0 * z * delta(i,2) * Sigma + z2 * dSigmadx[i] + 2.0 * R * dRdx[i] * rho2 * gamma_RR + 2.0 * R2 * rho * drhodx[i] * gamma_RR + R2 * rho2 * dgammaRRdx[i]); 
