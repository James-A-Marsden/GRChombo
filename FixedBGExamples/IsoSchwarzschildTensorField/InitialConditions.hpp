...
class InitialConditions
{
  ...
  public:
    ...
    template <class data_t> void compute(Cell<data_t> current_cell) const
    {
        ...
        const data_t sech2 = (1.0/cosh((r - beta)/lam)) * (1.0 / cosh((r - beta)/lam));
       
        const data_t seed = (1e-5) * r5 * (1.0 + tanh1);
        const data_t d1seed =  (1e-5) * -r4 * (r * sech2 + 5.0 * lam * (1.0 + tanh1)) / lam;
        const data_t d2seed = (1e-5) * 2.0 * r3 * (10.0 * (1.0 + tanh1) + r * sech2 * (5.0 * lam - r * tanh1) / lam2);
        const data_t d3seed = (1e-5) * pow(lam,-3.0) * 2.0 * r2 * (r3 * sech2 * sech2 - 30.0 * lam2 * lam * (1.0 + tanh1) + r * sech2 * (-30.0 * lam2 + 15.0 * r * lam * tanh1 - 2.0 * r2 * tanh1 * tanh1));
        const data_t d4seed = (1e-5) * pow(lam, -4.0) * 8.0 * r * (15.0 * lam2 * lam2 * (1.0 + tanh1) + r3 * sech2 * sech2 * (-5.0 * lam + 2.0 * r * tanh1) + r * sech2 * (30.0 * lam2 * lam - 30.0 * r * lam2 * tanh1 + 10.0 * r2 * lam * tanh1 * tanh1 - r3 * tanh1 * tanh1 * tanh1));

        const data_t seed_momentum = -(1e-5) * (r5 * sech2 / lam + 5.0 * r4 * (1.0 + tanh1));
        const data_t d1seed_momentum = -(1e-5) * 2.0 * r3 * (-10.0 * (1.0 + tanh1) + r * sech2 * (-5.0 * lam + r * tanh1) / lam2);
        const data_t d2seed_momentum = -(1e-5) * pow(lam,-3.0) * 2.0 * r2 * (-r3 * sech2 * sech2 + 30.0 * lam2 * lam * (1.0 + tanh1) + r * sech2 * (30.0 * lam2 - 15.0 * r * lam * tanh1 + 2.0 * r2 * tanh2));
        const data_t d3seed_momentum = -(1e-5) * -pow(lam,-4.0) * 8.0 * r * (15.0 * lam2 * lam2 * (1.0 + tanh1) + r3 * sech2 * sech2 * (-5.0 * lam + 2.0 * r * tanh1) + r * sech2 * (30.0 * lam2 * lam - 30.0 * r * lam2 * tanh1 + 10.0 * r2 * lam * tanh2 - r3 * tanh2 * tanh1));
        const data_t d4seed_momentum = -(1e-5) * pow(lam,-5.0) * 8.0 * (2.0 * r5 * pow(sech2,3.0) + 15.0 * pow(lam,5.0) * (1.0 + tanh1) + r3 * sech2 * sech2 * (-50.0 * lam2 + 50.0 * r * lam * tanh1 - 11.0 * r2 * tanh2) + r * sech2 * (75.0 * lam2 * lam2 - 150.0 * r * lam2 * lam * tanh1 + 100.0 * r2 * lam2 * tanh2 - 25.0 * r3 * lam * tanh2 * tanh1 + 2.0 * r4 * tanh2 * tanh2));

        /*
        const data_t seed = exp(-pow(r-beta,2.0) * pow(lam,-2.0)/2.0);
        const data_t d1seed = exp(-pow(r-beta,2.0) * pow(lam,-2.0)/2.0) * (r - beta) * pow(lam,-2.0);
        const data_t d2seed = exp(-pow(r-beta,2.0) * pow(lam,-2.0)/2.0) * (-lam * lam + pow(beta - r,2.0)) * pow(lam,-4.0);
        const data_t d3seed = exp(-pow(r-beta,2.0) * pow(lam,-2.0)/2.0) * (3.0 * lam * lam -  pow(beta-r,2.0)) * (beta-r) * pow(lam,-6.0);
        const data_t d4seed = exp(-pow(r-beta,2.0) * pow(lam,-2.0)/2.0) * (3.0 * pow(lam,4.0) - 6.0 * lam * lam * pow(beta-r,2.0) + pow(beta-r,4.0)) * pow(lam,-8.0);

        const data_t seed_momentum = exp(-pow(r-beta,2.0) * pow(lam,-2.0)/2.0) * (r - beta);
        const data_t d1seed_momentum = exp(-pow(r-beta,2.0) * pow(lam,-2.0)/2.0) * (-lam * lam * pow(beta-r,2.0)) * pow(lam,-2.0);
        const data_t d2seed_momentum = exp(-pow(r-beta,2.0) * pow(lam,-2.0)/2.0) * -1.0 * (-3.0 * lam * lam + pow(beta - r,2.0)) * (beta - r) * pow(lam,-4.0);
        const data_t d3seed_momentum = exp(-pow(r-beta,2.0) * pow(lam,-2.0)/2.0) * (3.0 * pow(lam,4.0) - 6.0 * lam * lam * pow(beta - r,2.0) + pow(beta-r,4.0)) * pow(lam,-6.0);
        const data_t d4seed_momentum = exp(-pow(r-beta,2.0) * pow(lam,-2.0)/2.0) * -1.0 * (15.0 * pow(lam,4.0) - 10.0 * lam * lam * pow(beta-r,2.0) + pow(beta-r,4.0)) * (beta-r) * pow(lam,-8.0);
        */

        /*
        const data_t seed = exp(-pow(r-beta,2.0)) * r5;
        const data_t d1seed = exp(-pow(r-beta,2.0)) * r4 * (-5.0 * lam + 2.0 * r * (r - beta) ) / lam;
        const data_t d2seed = exp(-pow(r-beta,2.0)) * 2.0 * r3 * (10.0 * lam * lam + lam *(10.0 * beta - 11.0 * r) * r + 2.0 * pow(beta-r,2.0) * r2) * pow(lam,-2.0);
        const data_t d3seed = exp(-pow(r-beta,2.0)) * -2.0 * r2 * (30.0 * pow(lam,3.0) + 15.0 * lam * lam * (4.0 * beta - 5.0 * r) * r + 4.0 * pow(beta-r,3.0) * r3 + 6.0 * lam * r2 * (5.0 * beta * beta - 11.0 * beta * r + 6.0 * r2)) * pow(lam,-3.0);
        const data_t d4seed = exp(-pow(r-beta,2.0)) * 4.0 * r * (30.0 * pow(lam,4.0) + 60.0 * pow(lam,3.0) * (2.0 * beta - 3.0 * r) * r + 4.0 * lam * (10.0 * beta - 13.0 * r) * pow(beta - r,2.0) * r3 + 4.0 * pow(beta-r,4.0) * r4 + 3.0 * lam * lam * r2 * (40.0 * beta * beta - 100.0 * beta * r + 61.0 * r2)) * pow(lam,-4.0);
        */
        /*
        const data_t seed = exp(-pow(r-1.0,2.0)/lam) * (r-1.0);
        const data_t d1seed = exp(-pow(r-1.0,2.0)/lam) * (-lam + 2.0 * pow(r - 1.0,2.0) )/ lam;
        const data_t d2seed = exp(-pow(r-1.0,2.0)/lam) * 2.0 * (-3.0 * lam + 2.0 * pow(r - 1.0,2.0)) * (r-1.0) * pow(lam,-2.0);
        const data_t d3seed = exp(-pow(r-1.0,2.0)/lam) * (6.0 * lam * lam - 24.0 * lam * pow(r-1.0,2.0) + 8.0 * pow(r -1.0,4.0)) * pow(lam,-3.0);
        const data_t d4seed = exp(-pow(r-1.0,2.0)/lam) * 4.0 * (15.0 * lam * lam - 20.0 * lam * pow(r-1.0,2.0) + 4.0 * pow(r-1.0,4.0)) * (r-1.0) * pow(lam,-4.0);
        */

        /*        
        const data_t A_func = 3.0 * (d2seed/r3 + 3.0 * d1seed/r4 + 3.0 * seed/r5);
        const data_t B_func = - 1.0 * (d3seed/r2 + 3.0 * d2seed/r3 + 6.0 * d1seed/r4 + 6.0 * seed/r5);
        const data_t C_func = 0.25 * (d4seed/r + 2.0 * d3seed/r2 + 9.0 * d2seed/r3 + 21.0 * d1seed/r4 + 21.0 * seed/r5);

        const data_t A_func_momentum = 3.0 * (d2seed_momentum/r3 + 3.0 * d1seed_momentum/r4 + 3.0 * seed_momentum/r5);
        const data_t B_func_momentum = - 1.0 * (d3seed_momentum/r2 + 3.0 * d2seed_momentum/r3 + 6.0 * d1seed_momentum/r4 + 6.0 * seed_momentum/r5);
        const data_t C_func_momentum = 0.25 * (d4seed_momentum/r + 2.0 * d3seed_momentum/r2 + 9.0 * d2seed_momentum/r3 + 21.0 * d1seed_momentum/r4 + 21.0 * seed_momentum/r5);
        ...
        spherical_soln[2][2] = r2 * sintheta * sintheta * (C_func * fpp_1 + A_func * fpp_2);
        spherical_soln[0][2] = 0.0;
        ...
        spherical_momentum[1][1] = r2 * (C_func_momentum * ftt_1 + A_func_momentum * ftt_2);
        spherical_momentum[1][2] = 0.0;
        spherical_momentum[2][2] = r2 * sintheta * sintheta * (C_func_momentum * fpp_1 + A_func_momentum * fpp_2);
        spherical_momentum[0][2] = 0.0;
        ...
        FOR2(i,j)
        {
          FOR2(k,l)
          {
            vars.fspatial[i][j] += sph_to_cart_J_UD[k][i] * sph_to_cart_J_UD[l][j] * spherical_soln[k][l];
            vars.v[i][j] += sph_to_cart_J_UD[k][i] * sph_to_cart_J_UD[l][j] * spherical_momentum[k][l];
          }
        }
        */
        ...
        const data_t wall_deriv = tanh(shift + z) * pow(cosh(shift - z),-2.0) + tanh(shift - z) * pow(cosh(shift + z),-2.0);
        ...
        */
        ...
        const data_t wall_deriv = 0.5 * (pow(cosh(z-shift),-2.0) - pow(cosh(z+shift),-2.0));
        const data_t field_init = wall;
        const data_t momentum_init = wall_deriv;
        */
        //Wall 3 (single wall)
