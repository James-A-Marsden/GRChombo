...
template <class matter_t, class background_t> class TraceFieldRemoval
{
    ...
    // FixedBGTensorField<TensorPotential>::template Vars<data_t>;
  ...
  public:
    TraceFieldRemoval(background_t a_background) : m_background(a_background) {}
    ...
    template <class data_t> void compute(const Cell<data_t> current_cell) const
    {
        ...
        // template <class data_t> using MetricVars = typename
        // ADMFixedBGVars::template Vars<data_t>;
        ...
        // TensorAlgebra::make_trace_free(local_vars.fspatial,
        // metric_vars.gamma, gamma_UU);
        // TensorAlgebra::make_trace_free(local_vars.v, metric_vars.gamma,
        // gamma_UU);

        // auto ftrace = TensorAlgebra::compute_trace(local_vars.fspatial,
        // gamma_UU); auto vtrace = TensorAlgebra::compute_trace(local_vars.v,
        // gamma_UU); double one_over_gr_spacedim = 1. / ((double)GR_SPACEDIM);

        // local_vars.fspatial[0][0] -= (1.0 / 2.0) * metric_vars.gamma[0][0] *
        // ftrace; local_vars.fspatial[1][1] -= (1.0 / 2.0) *
        // metric_vars.gamma[1][1] * ftrace;
...
GRInterval<c_fspatial11, c_fspatial33>(), h);
