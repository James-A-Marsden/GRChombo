...
class SetRest
{
  ...
  public:
    ...
    template <class data_t> void compute(Cell<data_t> current_cell) const
    {
        ...
        // double bessel = -100.0 * zs * bmath::cyl_bessel_j(1,-zs *
        // frequency);// - zs * boost::math::cyl_neumann(1,zs * frequency);
        double bessel =
            -zs * frequency * (4.0 / 3.0) * pow(coords.z + 100.0, 1.0 / 3.0) *
                bmath::cyl_bessel_j(1, zs * frequency) +
            zs * (4.0 / 3.0) / (coords.z + 100.0) *
                bmath::cyl_bessel_j(
                    0, zs * frequency); // - zs * boost::math::cyl_neumann(1,zs
                                        // * frequency);
