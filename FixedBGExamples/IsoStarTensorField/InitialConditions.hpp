...
class InitialConditions
{
  ...
  public:
    ...
    template <class data_t> void compute(Cell<data_t> current_cell) const
    {
        ...
        sintheta; vars.q[1] = 64.0 * pow(r, 3.0) * cosphi * pow(M + 2.0 * r,
        -5.0)  * sintheta; vars.q[2] = 0.0;
        */
        /*
        vars.fspatial[0][0] =   sin2theta * cosphi * sintheta * sintheta * r2 *
        exp(-r); vars.fspatial[1][1] =  -sin2theta * cosphi * sintheta *
        sintheta * exp(-r); vars.fspatial[2][2] =  0.0; vars.fspatial[0][1] =
        cos2theta * cosphi * sintheta * sintheta r * exp(-r);
        ...
        */
        ...
        data_t A;
        data_t initial_coefficient = (M / (M + 2.0 * R)) / (C3 + 2.0 * M2 / R2);
        if (simd_compare_gt(r, M / C))
        {
            A = M * sintheta * sintheta * pow(M + 2.0 * r, -2.0);
        }
        else
        {
            A = initial_constant * sintheta * sintheta * (C3 + 2.0 * M2 / r2);
        }
        ...
        */
        ...
        // vars.q[0] =  -64.0 * pow(r,3.0) * sintheta * sinphi * pow(M + 2.0 *
        // r, -5.0); vars.q[1] = 64.0 * pow(r,3.0) * sintheta * cosphi * pow(M
        // + 2.0 * r, -5.0); vars.q[2] = 0.0;

        // vars.q[0] = 16.0 * exp(-r) * y * pow(r,3.0) * (2.0 * (r2 - 4.0 * r) +
        // M * (-2.0 + r)) * pow(M + 2.0 * r,-5.0); vars.q[1] = -16.0 * exp(-r)
        // * x * pow(r,3.0) * (2.0 * (r2 - 4.0 * r) + M * (-2.0 + r)) * pow(M
        // + 2.0 * r,-5.0);

        // vars.q[0] = 16.0 * exp(-r) * y * pow(r,3.0) * (2.0 * (r2 - 4.0 * r) +
        // M * (-2.0 + r)) * pow(M + 2.0 * r,-5.0);

        // vars.q[0] = 16.0 * exp(-r) * r2 * r2 *(M * (-2.0 + r) + 2.0 * (-4.0 +
        // r)*r) * sinphi * pow(M + 2.0 * r, -5.0) * sintheta; vars.q[0] = 16.0
        // * exp(-r) * y * pow(r,3.0) * (2.0 * (r2 - 4.0 * r) + M * (-2.0 + r))
        // * pow(M + 2.0 * r,-5.0);

        // vars.q[0] = 16.0 * exp(-r) * r2 * r2 *(M * (-2.0 + r) + 2.0 * (-4.0 +
        // r)*r) * sinphi * pow(M + 2.0 * r, -5.0) * sintheta; vars.q[1] = -16.0
        // * exp(-r) * r2 * r2 *(M * (-2.0 + r) + 2.0 * (-4.0 + r)*r) * cosphi *
        // pow(M + 2.0 * r, -5.0) * sintheta; vars.q[2] = 0.0;

        // vars.q[0] =  16.0 * exp(-r) * r2 * r *(-4.0 + M + 2.0*r) * sinphi *
        // pow(M + 2.0 * r, -5.0) * sintheta; vars.q[1] = -16.0 * exp(-r) * r2 *
        // r *(-4.0 + M + 2.0*r) * cosphi * pow(M + 2.0 * r, -5.0) * sintheta;
        // vars.q[2] = 0.0;
        ...
        // vars.q[0] = 16.0 * exp(-r) * y * pow(r,3.0) * (-4.0 + r) * pow(M
        // + 2.0 * r, -4.0); vars.q[1] = -16.0 * exp(-r) * x * pow(r,3.0) *
        // (-4.0 + r) * pow(M + 2.0 * r, -4.0); vars.q[2] = 0.0;

        // vars.q[0] = 32.0 * exp(-r) * M * y * pow(r,3.0) * pow(M + 2.0 * r,
        // -5.0); vars.q[1] = -32.0 * exp(-r) * M * x * pow(r,3.0) * pow(M + 2.0
        // * r, -5.0); vars.q[2] = 0.0; vars.q[0] =  16.0 * exp(-r) * r2 * r
        // *(-4.0 + M + 2.0*r) * sinphi * pow(M + 2.0 * r, -5.0) * sintheta;
        // vars.q[1] = -16.0 * exp(-r) * r2 * r *(-4.0 + M + 2.0*r) * cosphi *
        // pow(M + 2.0 * r, -5.0) * sintheta; vars.q[2] = 0.0;//(2.0 * x) *
        // exp(-r) - x * r * exp(-r);
        ...
        vars.q[2] = r;
        */
        // vars.q[2] = //- gamma_UU[0][0] * 2.0 * exp(-r) * y *(y2 + z2 -x2 *r)
        // * pow(r,-3.0); vars.q[2] = -gamma_UU[1][1] * pow(rho,-4.0) * exp(r) *
        // y * (-pow(x,6.0) + pow(x,4.0) * (-y2 -z2 + r) + y2 * y2 *(y2 + z2
        // + 3.0 * r) + x2 * (y2 * y2 + 4.0 * y2 * r + 4.0 * z2 * r));
        ...
        // vars.q[0] =  -64.0 * exp(-r) * pow(r,5.0) * sintheta * sinphi * pow(M
        // + 2.0 * r,-5.0); vars.q[1] = 64.0 * exp(-r) * pow(r,5.0) * sintheta *
        // cosphi * pow(M + 2.0 * r,-5.0); vars.q[0] =  16.0 * exp(-r) * (-2.0 +
        // r) * pow(r,4.0) * sintheta * sinphi * pow(M + 2.0 * r,-4.0);
        // vars.q[1] = -16.0 * exp(-r) * (-2.0 + r) * pow(r,4.0) * sintheta *
        // cosphi * pow(M + 2.0 * r,-4.0); vars.q[0] =  32.0 * exp(-r2) *
        // pow(r, 4.0) * (M * (-1.0 * r2) + 2.0 * r * (-2.0 + r2)) * sintheta *
        // sinphi * pow(M + 2.0 * r, -5.0); vars.q[1] = -32.0 * exp(-r2) *
        // pow(r, 4.0) * (M * (-1.0 * r2) + 2.0 * r * (-2.0 + r2)) * sintheta *
        // cosphi * pow(M + 2.0 * r, -5.0); vars.q[2] = 0.0;
        ...
        vars.fhat = TensorAlgebra::compute_trace(gamma_UU, vars.fspatial);
        vars.fhat = 0.0;
        //vars.q[0] = 128.0 * pow(r,4.0) * sintheta * sinphi * pow(M + 2.0 * r,
        -5.0) / (M - 2.0 * r);
        //vars.q[1] = -128.0 * pow(r,4.0) * sintheta * cosphi * pow(M + 2.0 * r,
        -5.0) / (M - 2.0 * r); vars.q[0] = 0.0;//-64.0 * pow(r, 3.0) * sinphi *
        pow(M + 2.0 * r, -5.0)  * sintheta; vars.q[1] = 0.0;//64.0 * pow(r, 3.0)
        * cosphi * pow(M + 2.0 * r, -5.0)  * sintheta; vars.q[2] = 0.0;
        //vars.q[2] = -128.0 * pow(r,5.0) * sintheta * sintheta * pow(M + 2.0 *
        r, -5.0) / (M - 2.0 * r) ;
        */
